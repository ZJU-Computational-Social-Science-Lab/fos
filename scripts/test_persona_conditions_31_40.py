...
(orig), 1, 2, or 3 on one of three models, saving every decision immediately to
...
- Parse amount/reason (first "{" to last "}" as JSON), reasoning_content,
  ...
  finish_reason, raw content, elapsed seconds, succeeded (amount in 31..40),
  error.
...
# The 31-40 game text shared by all conditions (condition 1 uses it plain;
...
_GAME_TEXT = (
    ...
    "The amount must be (an integer) between 31 and 40 shekels. Each player will receive the amount he requests. "
...
_REASON_INSTRUCTION = (
    ...
    "What amount of money would you request? (must be an integer between 31 and 40)\n\n"
...
class DecisionRecord:
    """One persona's decision in the 31-40 game under one condition."""
...
def build_record(
    ...
    try:
        ...
        record.succeeded = chosen_number is not None and 31 <= chosen_number <= 40
...
def results_jsonl_path(model: str, condition: int) -> Path:
    """The results JSONL path for one model+condition (results/..._condN_31-40.jsonl)."""
    return _REPO_ROOT / "results" / f"{model.replace('/', '_')}_cond{condition}_31-40.jsonl"


def results_csv_path(model: str, condition: int) -> Path:
    """The results CSV path for one model+condition (results/..._condN_31-40.csv)."""
    return _REPO_ROOT / "results" / f"{model.replace('/', '_')}_cond{condition}_31-40.csv"
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Run the 31-40 Money Request Game for personas under prompt "
            "condition 0 (orig), 1, 2, or 3 on one model. Use --dry-run to "
            "print the prompts for all four conditions without calling any API."
        )
    )
    parser.add_argument(
        "--model",
