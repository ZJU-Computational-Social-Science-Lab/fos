...
def load_all_data() -> pd.DataFrame:
    """Read all per-agent CSV files across seeds/networks. Handles missing subdirs."""
    rows = []
    for seed in NUM_SEEDS:
        ...
        if not sd.is_dir():
            ...
            continue
        subdirs = {
            d.name[len("council_100agents_") :].lower(): d / "combined_results.csv"
            for d in sd.iterdir()
            if d.is_dir() and d.name.startswith("council_100agents_")
        }
        top_csv = sd / "combined_results.csv"
        for net in NETWORKS:
            if net in subdirs and subdirs[net].is_file():
                csv_path = subdirs[net]
            elif top_csv.is_file():
                csv_path = top_csv
                print(
                    f"  [WARN] No subdir for {net} in seed {seed}, using top-level CSV"
                )
            else:
                print(f"  [WARN] No data for '{net}' in seed {seed}, skip")
                continue
            dnet = pd.read_csv(csv_path)
...
def fig1_yesrate(df):
    ...
    pv, _ = _voting_pivot(df[df["action"].isin(["vote_yes", "vote_no"])])
    pv["yes_rate"] = pv["vote_yes"] / (pv["vote_yes"] + pv["vote_no"] + 1e-10)
...
def fig3_model_rates(df):
    """Grouped bar: 5 models × 3 networks. Yes-rate = yes/(yes+no). Replaces old."""
    pv, _ = _voting_pivot(
        df[df["action"].isin(["vote_yes", "vote_no"])], ["model_short"]
    )
    pv["yes_rate"] = pv["vote_yes"] / (pv["vote_yes"] + pv["vote_no"] + 1e-10)
    snm = (
...
# ── Figure 5 ──
def fig6_placebo(df):
    """Placebo: histogram of null Spearman rho (shuffled degrees) vs observed value."""
    voting = df[df["action"].isin(["vote_yes", "vote_no"])]
    pv, _ = _voting_pivot(voting)
    pv["yes_rate"] = pv["vote_yes"] / 100  # yes / total_agents
    ar = (
        pv.groupby(["seed", "network_label", "agent_id"])["yes_rate"]
        .mean()
        .reset_index()
    )
    ad = df[["seed", "network_label", "agent_id", "degree"]].drop_duplicates()
    merged = ar.merge(ad, on=["seed", "network_label", "agent_id"])
    rho_obs, _ = stats.spearmanr(merged["degree"], merged["yes_rate"])
    rng = np.random.default_rng(42)
    null_rhos = np.empty(SHUFFLE_COUNT)
    for i in range(SHUFFLE_COUNT):
        ads = ad.copy()
        ads["degree"] = ads.groupby(["seed", "network_label"], group_keys=False)[
            "degree"
        ].transform(lambda x: x.sample(frac=1, random_state=rng).values)
        mn = ar.merge(ads, on=["seed", "network_label", "agent_id"])
        r, _ = stats.spearmanr(mn["degree"], mn["yes_rate"])
        null_rhos[i] = r
    pv_ = np.mean(null_rhos >= rho_obs)
    ci_l, ci_h = np.percentile(null_rhos, [2.5, 97.5])
    ...
    ax.hist(
        ...
        color="gray",
        edgecolor="white",
        label=f"Null (shuffled, N={SHUFFLE_COUNT})",
    )
    ax.axvline(rho_obs, color="#A60628", ls="--", lw=2, label=f"Observed ρ = {rho_obs:.3f}")
    ax.axvline(
        ci_l, color="gray", ls=":", alpha=0.6
    )
    ...
    print(f"    (ρ_obs={rho_obs:.3f}, p={pv_:.4f}, CI=[{ci_l:.3f}, {ci_h:.3f}])")


# ── Figure 6 ──
def figC_strengthened(df):
    """Logistic regression: P(Yes) vs k agreeing neighbours, controlling for Big5 + archetype."""
    voting = df[df["action"].isin(["vote_yes", "vote_no"])]
    cols = ["seed", "network_label", "proposal", "model_short", "agent_id", "action"]
    vc = voting.groupby(cols).size().reset_index(name="n")
    ac = voting.groupby(["seed", "network_label", "proposal", "model_short", "action"])["agent_id"].nunique().reset_index(name="action_count")
    vc = vc.merge(ac, on=["seed", "network_label", "proposal", "model_short", "action"])
    vc["k"] = vc["action_count"] - 1
    at = df[["seed", "network_label", "agent_id", "Openness", "Conscientiousness",
             "Extraversion", "Agreeableness", "Neuroticism", "archetype_label"]].drop_duplicates()
    md = vc.merge(at, on=["seed", "network_label", "agent_id"])
    md["is_yes"] = (md["action"] == "vote_yes").astype(int)
    try:
        formula = (
