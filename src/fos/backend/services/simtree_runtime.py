...
def _scene_config_scenario_id(cfg: dict | None) -> str:
    """Return the direct or nested experiment scenario id from a scene config."""
    if not isinstance(cfg, dict):
        return ""
    nested = cfg.get("generic_config")
    if isinstance(nested, dict):
        scenario_id = str(nested.get("scenario_id") or "").strip()
        if scenario_id:
            return scenario_id
    return str(cfg.get("scenario_id") or "").strip()


def _effective_policy_cascade_config(cfg: dict | None) -> dict:
    """Return policy cascade config from top-level or nested generic_config."""
    if not isinstance(cfg, dict):
        return {}
    nested = cfg.get("generic_config")
    if isinstance(nested, dict) and _scene_config_scenario_id(cfg) == "policy_erosion":
        merged = {**nested, **{key: value for key, value in cfg.items() if key != "generic_config"}}
        nested_params = nested.get("parameters") if isinstance(nested.get("parameters"), dict) else {}
        top_params = cfg.get("parameters") if isinstance(cfg.get("parameters"), dict) else {}
        merged["parameters"] = {**nested_params, **top_params}
        return merged
    return cfg
...
def _build_tree_for_sim(sim_record, clients: dict | None = None) -> SimTree:
    ...
    if _scene_config_scenario_id(cfg) == "policy_erosion":
    ...
    if scene_key == "policy_cascade_scene":
        policy_cfg = _effective_policy_cascade_config(cfg)
        params = policy_cfg.get("parameters") or {}
    ...
    if scene_key in {"simple_chat_scene", "emotional_conflict_scene"}:
        ...
        if scene_key == "policy_cascade_scene" and hasattr(
            ...
            scene.configure_from_config(_effective_policy_cascade_config(cfg))

    # 存储社交网络拓扑到场景状态中（如果配置了的话）
    scene_cfg_for_network = _effective_policy_cascade_config(cfg) if scene_key == "policy_cascade_scene" else cfg
    social_network = scene_cfg_for_network.get("social_network") or {}
    if social_network:
